    ...
    for vsc = 1:1:size(T_VSC.bus,1) 
        ...
        switch mode
            ...
            % -------------------------------------------------------------
                ...
                vgq_zoh_x = {vgq_zoh_x1;vgq_zoh_x2};
                ...
                vgd_zoh_x = {vgd_zoh_x1;vgd_zoh_x2};
                ...
                vg_g2l   = build_global2local(vg_g2l_x,vg_g2l_u,vg_g2l_y,e_theta0,vg_q0,vg_d0);

                ss_list{end+1} = vg_g2l ;

                % %vgq measurement delay
                % vgq_md_u = vg_g2l_y{1};
                % vgq_md_y = {'vg_qc_pre_zoh'};
                % %vgq_measurement_delay = build_measurement_delay(vgq_md_x,vgq_md_u,vgq_md_y,tau_md);
                % x = {['WT' num2str(num) '.vgq_btw1'];['WT' num2str(num) '.vgq_btw2']};
                % vgq_measurement_delay = build_btw_filter(1.853539665617978e+04,2,x,vgq_md_u,vgq_md_y);
                % 
                % ss_list{end+1} = vgq_measurement_delay;
                % 
                % %vgd measurement delay
                % vgd_md_u = vg_g2l_y{2};
                % vgd_md_y = {'vg_dc_pre_zoh'};
                % %vgd_measurement_delay = build_measurement_delay(vgd_md_x,vgd_md_u,vgd_md_y,tau_md);
                % x = {['WT' num2str(num) '.vgd_btw1'];['WT' num2str(num) '.vgd_btw2']};
                % vgd_measurement_delay = build_btw_filter(1.853539665617978e+04,2,x,vgd_md_u,vgd_md_y);
                % 
                % ss_list{end+1} = vgd_measurement_delay;

                % vg BTWF abc:
                %OJO 50hz!!!!
                u = {vg_g2l_y{1};vg_g2l_y{2}};
                ...
                btw_filter = build_btw_filter_abc(fc,x_q,x_d,u,y_vg_btwf);
                ss_list{end+1} = btw_filter;

                % %vgq ZOH delay
                % vgq_zoh_u = vgq_md_y;
                % vgq_zoh_y = {'vg_qc'};
                % vgq_zoh_delay = build_zoh2_2order(vgq_zoh_x,vgq_zoh_u,vgq_zoh_y,tau_zoh);
                % 
                % ss_list{end+1} = vgq_zoh_delay;
                % 
                % %vgd ZOH delay
                % vgd_zoh_u = vgd_md_y;
                % vgd_zoh_y = {'vg_dc'};
                % vgd_zoh_delay = build_zoh2_2order(vgd_zoh_x,vgd_zoh_u,vgd_zoh_y,tau_zoh);
                % 
                % ss_list{end+1} = vgd_zoh_delay;
                ...
                u = y_vg_btwf;
                y = {'vg_qc';'vg_dc'};
                zoh_delay = build_zoh_abc_2order(x_q,x_d,u,y,tau_zoh);
                ...
                % isq_md_y = {'is_qc_pre_zoh'};
                % %isq_measurement_delay = build_measurement_delay(isq_md_x,isq_md_u,isq_md_y,tau_md);
                % x = {['WT' num2str(num) '.isq_btw1'];['WT' num2str(num) '.isq_btw2']};
                % isq_measurement_delay = build_btw_filter(1.853539665617978e+04,2,x,isq_md_u,isq_md_y);
                % 
                % ss_list{end+1} = isq_measurement_delay;
                % 
                % %isd measurement delay
                ...
                % isd_md_y = {'is_dc_pre_zoh'};
                % %isd_measurement_delay = build_measurement_delay(isd_md_x,isd_md_u,isd_md_y,tau_md);
                % x = {['WT' num2str(num) '.isd_btw1'];['WT' num2str(num) '.isd_btw2']};
                % isd_measurement_delay = build_btw_filter(1.853539665617978e+04,2,x,isd_md_u,isd_md_y);
                % 
                % ss_list{end+1} = isd_measurement_delay;

                % is BTWF abc:
                u = {is_g2l_y{1};is_g2l_y{2}};
                ...
                btw_filter = build_btw_filter_abc(fc,x_q,x_d,u,y_is_btwf);
                ss_list{end+1} = btw_filter;
                ...
                % igq_md_y = {'ig_qc_pre_zoh'};
                % %igq_measurement_delay = build_measurement_delay(igq_md_x,igq_md_u,igq_md_y,tau_md);
                % x = {['WT' num2str(num) '.igq_btw1'];['WT' num2str(num) '.igq_btw2']};
                % igq_measurement_delay = build_btw_filter(1.853539665617978e+04,2,x,igq_md_u,igq_md_y);
                % 
                % ss_list{end+1} = igq_measurement_delay;
                % 
                % %igd measurement delay
                ...
                % igd_md_y = {'ig_dc_pre_zoh'};
                % %igd_measurement_delay = build_measurement_delay(igd_md_x,igd_md_u,igd_md_y,tau_md);
                % x = {['WT' num2str(num) '.igd_btw1'];['WT' num2str(num) '.igd_btw2']};
                % igd_measurement_delay = build_btw_filter(1.853539665617978e+04,2,x,igd_md_u,igd_md_y);
                % 
                % ss_list{end+1} = igd_measurement_delay;

                % ig BTWF abc:
                u = {ig_g2l_y{1};ig_g2l_y{2}};
                ...
                btw_filter = build_btw_filter_abc(fc,x_q,x_d,u,y_ig_btwf);
                ss_list{end+1} = btw_filter;
                ...
                % uq_md_y = {'u_qc_pre_zoh'};
                % %uq_measurement_delay = build_measurement_delay(uq_md_x,uq_md_u,uq_md_y,tau_md);
                % x = {['WT' num2str(num) '.uq_btw1'];['WT' num2str(num) '.uq_btw2']};
                % uq_measurement_delay = build_btw_filter(1.853539665617978e+04,2,x,uq_md_u,uq_md_y);
                % ss_list{end+1} = uq_measurement_delay;
                % 
                % %ud measurement delay
                % ud_md_u = u_g2l_y{2};
                % ud_md_y = {'u_dc_pre_zoh'};
                % %ud_measurement_delay = build_measurement_delay(ud_md_x,ud_md_u,ud_md_y,tau_md);
                % x = {['WT' num2str(num) '.ud_btw1'];['WT' num2str(num) '.ud_btw2']};
                % ud_measurement_delay = build_btw_filter(1.853539665617978e+04,2,x,ud_md_u,ud_md_y);
                % ss_list{end+1} = ud_measurement_delay;

                % u BTWF abc:
                u = {u_g2l_y{1};u_g2l_y{2}};
                ...
                btw_filter = build_btw_filter_abc(fc,x_q,x_d,u,y_u_btwf);
                ss_list{end+1} = btw_filter;
                ...
                u = {'u_dc'};
                y = {'u_dc_pll_filt'};
                pll_mesurement_filter = build_measurement_delay(x,u,y,0);
                ...
                u = {'vg_qc' 'vg_dc'};
                y = {'Umag'};
                ss_u = build_voltage_magnitude(x,u,y,vg_qc0,vg_dc0);
                ...
                % Ptodc calculation
                p_x={''};
                p_u={is_q is_d,'vc_q','vc_d'};
                p_y={['WT',num2str(num),'.p']};
                ss_p = build_active_power_todc_calculation(p_x,p_u,p_y,is_q0,is_d0,vc_q0,vc_d0);
                ...
                ss_q = build_reactive_power_calculation(q_x_calc,q_u,q_y,ig_qc0,ig_dc0,vg_qc0,vg_dc0);
                ...
                % DC voltage control
                ss_idc = WT_SS_DCPower(2, 3/2*vc_q0*is_q0+3/2*vc_d0*is_d0, num);
                ss_list{end+1} = ss_idc;
                ...
                % divide by DC voltage
                % ref_u = is_y{1};
                % ref_y = {'vc_qdc'};
                % ref_q = build_ref_q_dc(ref_u,ref_y,vc_qc0,2);
                % 
                % % Commutation delay q
                % %cmd_u = zoh_vdiffq_y;
                % cmd_u = is_y{1};
                ...
                y = {'vc_qc_m';'vc_dc_m'};
                modulation = build_modulation(x,u,y,2,vc_qc0,vc_dc0);
                ...
                u = {'vc_qc_m';'vc_dc_m'};
                y = {'vc_qc','vc_dc'};
                com_delay = build_com_delay_2order_abc(x_q,x_d,u,y,tau_cmd);
